...
def put_person(pgsdata, pd):
    return conf.send_some(pgsdata, f"INSERT INTO people(tg_person, name, phone, description, photo) VALUES ({pd[0]},'{pd[1]}','{pd[2]}','{pd[3]}','{pd[4]}')")

def alt_person(pgsdata, pd):
    return conf.send_some(pgsdata, f"UPDATE people SET (name, phone, description, photo) = ('{pd[1]}', '{pd[2]}', '{pd[3]}', '{pd[4]}') WHERE tg_person={str(pd[0])}")
